...
## 共振突破系统 -- 使用中
- 逻辑概要
  - 主抓 板块启动上升前中期 板块中有协同板块明显拉升的标的 
- 必要条件
  - 板块条件
    - 启动前中期 && 板块K线上升 && 近日净流入持续且量大 && 非直线拉升(板块分时)
  - 个股条件
    - 板块逻辑正(排除蹭概念的个股) && 排除最近两日连续下跌 && 过去20个交易日内出现过单日涨幅过7%的阳线 && MA40平稳 && 10日线平稳 && 非必要的加分项(有回踩再新高)
- 入场信号
  - **一或多次出现在涨速榜前排 + 持续爆量 + 非直线拉升(个股分时) + ** && (**当日价格突破前2天最高点** || **破前阻力位**)
...
- 仓位管理（资金曲线控制）
  ...
  - 加仓：橄榄型加仓法
- 注意事项（避坑指南）
  - 避开财报窗口期/大盘单边下跌/**大盘差上午下午都延后45分钟操作**/同板块内的同时拉起的个股多观察哪个更强/摇摆不定的不做
